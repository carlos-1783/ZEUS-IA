...
def get_customer_or_404(
    ...
    Returns:
        CustomerOut: El objeto del cliente en formato Pydantic si se encuentra
...
def get_customer(
    ...
    """
    # Get the customer (already converted to CustomerOut by get_customer_or_404)
    customer = get_customer_or_404(db, customer_id, current_user)
    
    # Return the response using the response model
    return CustomerResponse(
        success=True,
        data=customer
    )
...
def delete_customer(
    ...
    """
    # Get the existing customer (already converted to CustomerOut by get_customer_or_404)
    customer = get_customer_or_404(db, customer_id, current_user)
    ...
    try:
        ...
        # In case of error, rollback and return an error
        db.rollback()
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
