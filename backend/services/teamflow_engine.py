...
def _build_workflows() -> Dict[str, TeamFlowWorkflow]:
    ...
    workflows: List[TeamFlowWorkflow] = [
        ...
        TeamFlowWorkflow(
            workflow_id="customer_offer_broadcast_v1",
            title="Oferta CRM → clientes por email",
            category="crm_marketing",
            summary="Crea campaña con descuento y notifica clientes del CRM (PERSEO + CRM + email).",
            success_criteria=[
                "Campaña registrada en actividad",
                "Clientes con email identificados",
                "Envíos registrados",
            ],
            entry_conditions=["Clientes en CRM con email", "Email (SendGrid/Resend) configurado"],
            default_payload={"target": "all_customers", "discount_percent": 5},
            sla_minutes=15,
            steps=[
                step(
                    id="crm_audience",
                    ...
                    description="Obtener y filtrar clientes con email en la empresa.",
                    expected_output="Lista de destinatarios válidos",
                    handoff_to=["campaign_create"],
                ),
                step(
                    id="campaign_create",
                    name="Crear campaña",
                    ...
                    description="Registrar campaña y mensaje en el sistema.",
                    depends_on=["crm_audience"],
                    expected_output="campaign_id + copy",
                    handoff_to=["notify_clients"],
                ),
                step(
                    id="notify_clients",
                    ...
                    description="Enviar emails y registrar actividad CRM.",
                    depends_on=["campaign_create"],
                    expected_output="Recuento de envíos exitosos",
                ),
            ],
        ),
        TeamFlowWorkflow(
