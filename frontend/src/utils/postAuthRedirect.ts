 ...
 * No bloquear por validation.ok (TPV técnico): basta con haber guardado el wizard.
 */
...
function isOnboardingMarkedDoneLocally(): boolean {
  try {
...
export async function resolvePostAuthPath(
  ...
  if (isOnboardingMarkedDoneLocally() && fallback !== '/onboarding-setup') {
    return fallback
  }
  ...
  try {
    ...
    if (isOnboardingMarkedDoneLocally()) {
      return fallback
    }
